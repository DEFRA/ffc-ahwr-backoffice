...
describe('Account page test', () => {
  const url = '/account'
  jest.mock('../../../../app/auth')
  let auth = { strategy: 'session-auth', credentials: { scope: ['administrator'], account: {} } }
  ...
  describe(`GET ${url} route`, () => {
    ...
    test.each([
      ...
      auth = { strategy: 'session-auth', credentials: { scope: roles, account: { name, username } } }
      const options = {
        method: 'GET',
        url,
        auth
      ...
      expect($('.govuk-summary-list__value').eq(2).text()).toMatch(roles.map(x => upperFirstLetter(x)).join(', '))
