...
describe("View Application test with Date of Testing enabled", () => {
  ...
  describe(`GET ${url} route`, () => {
    test("returns 200 application claim - claim date in application data", async () => {
      ...
      expect($(".govuk-summary-list__key").eq(3).text()).toMatch(
        "Email address",
      );
      ...
      expect($("#claim .govuk-summary-list__key").eq(1).text()).toContain(
        "Date of review",
      );
      expect($("#claim .govuk-summary-list__value").eq(1).text()).toContain(
        "07/11/2022",
      );
      expect($("#claim .govuk-summary-list__key").eq(2).text()).toContain(
        "Date of testing",
      );
      expect($("#claim .govuk-summary-list__value").eq(2).text()).toContain(
        "08/11/2022",
      );
      expect($("#claim .govuk-summary-list__key").eq(3).text()).toContain(
        "Date of claim",
      );
      ...
      expect($("#claim .govuk-summary-list__value").eq(4).text()).toContain(
        "Yes",
      );
      expect($("#claim .govuk-summary-list__key").eq(5).text()).toContain(
        "Vet’s name",
      );
      expect($("#claim .govuk-summary-list__value").eq(5).text()).toContain(
        "testVet",
      );
      expect($("#claim .govuk-summary-list__key").eq(6).text()).toContain(
        "Vet’s RCVS number",
      );
