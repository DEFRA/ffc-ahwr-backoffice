...
test('success: update claim', async () => {
  ...
  expect(res.headers.location)
    .toBe(`/view-${claimOrAgreement}/${reference}?page=${page}&returnPage=${returnPage}`)
})
...
test('success: update agreement', async () => {
  ...
  expect(res.statusCode).toBe(302)
  expect(res.headers.location)
    .toBe(`/view-${claimOrAgreement}/${reference}?page=${page}`)
})
...
test('failure: update claim, missing note', async () => {
  ...
  expect(res.statusCode).toBe(302)
  expect(res.headers.location)
    .toBe(`/view-${claimOrAgreement}/${reference}?page=${page}&updateStatus=true&errors=${encodedErrors}&returnPage=${returnPage}`)
})
...
test('failure: update agreement, missing note', async () => {
  ...
  expect(res.statusCode).toBe(302)
  expect(res.headers.location)
    .toBe(`/view-${claimOrAgreement}/${reference}?page=${page}&updateStatus=true&errors=${encodedErrors}`)
})
